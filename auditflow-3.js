(function () {
  ...
  function initSupabase() {
    ...
    sb.auth.onAuthStateChange((_e, session) => {
      ...
      syncUserUi();
      if (session?.user && window.location.pathname.includes('index.html')) {
  ...
  async function syncUserUi() {
    ...
    if (u && window.location.pathname.includes('auditflow.html')) {
      try {
        const { data: audits } = await sb.from('audits').select('id', { count: 'exact' }).eq('user_id', u.id);
        const { data: profile } = await sb.from('profiles').select('plan').eq('id', u.id).single();
        updateAuditCounter(audits?.length || 0, profile?.plan || 'free');
      } catch (e) {
        console.warn('Could not load audit counter:', e);
      }
    }
    icons();
  }
  ...
  async function runDeepAuditProtocol(pageUrl, html, timings, opts = {}) {
    // === AUDIT-LIMIT-CHECK (5 Audits für Free-Plan) ===
    if (!STATE.user) {
      toast(t('authNeedLogin'));
      return null;
    }
    
    try {
      // Zähle bestehende Audits des Users
      const { data: audits, error: auditError } = await sb.from('audits')
      ...
      // Limit-Check: Free-Plan hat max 5 Audits
      if (userPlan === 'free' && auditCount >= 5) {
        showPaywallModal(auditCount);
        return null;
      }
      
      // Update Counter in UI
      updateAuditCounter(auditCount, userPlan);
    } catch (e) {
      console.warn('Audit limit check failed:', e);
      // Fehler beim Limit-Check = trotzdem Audit erlauben (fallback)
    }
    // === END AUDIT-LIMIT-CHECK ===
  ...
  function renderDashboardResult(data, compare) {
    ...
    if (pr) {
      const prioritized = flat.filter(x => x.l === 'e' || x.l === 'w').slice(0, 15);
      pr.innerHTML = prioritized.length ? prioritized.map((f) => {
        const icon = f.l === 'e' ? '🔴' : '🟡';
        const cls = f.l === 'e' ? 'text-red-400' : 'text-amber-300';
        return `<li class="text-sm ${cls} mb-1.5 flex items-start gap-2"><span>${icon}</span><span>${escapeHtml(f.m)}</span></li>`;
      }).join('') : `<li class="text-zinc-500 text-sm">${t('noCritical')}</li>`;
    }
    ...
    if (act) {
      ...
      const grouped = { e: [], w: [], o: [] };
      items.forEach(i => {
        if (i.level === 'e') grouped.e.push(i);
        else if (i.level === 'w') grouped.w.push(i);
        else grouped.o.push(i);
      });
      let html = '';
      if (grouped.e.length > 0) {
        html += '<div class="mb-4"><p class="text-xs font-mono text-red-400 uppercase mb-2">🔴 Kritisch (' + grouped.e.length + ')</p>';
        html += grouped.e.map(i => '<div class="text-sm text-zinc-300 border-l-2 border-red-500 pl-3 py-1.5 mb-1">' + i.html + '</div>').join('');
        html += '</div>';
      }
      if (grouped.w.length > 0) {
        html += '<div class="mb-4"><p class="text-xs font-mono text-amber-300 uppercase mb-2">🟡 Warnung (' + grouped.w.length + ')</p>';
        html += grouped.w.map(i => '<div class="text-sm text-zinc-300 border-l-2 border-amber-400 pl-3 py-1.5 mb-1">' + i.html + '</div>').join('');
        html += '</div>';
      }
      if (grouped.o.length > 0) {
        html += '<div><p class="text-xs font-mono text-zinc-500 uppercase mb-2">ℹ️ Info (' + grouped.o.length + ')</p>';
        html += grouped.o.slice(0, 5).map(i => '<div class="text-sm text-zinc-400 border-l-2 border-zinc-700 pl-3 py-1.5 mb-1">' + i.html + '</div>').join('');
        if (grouped.o.length > 5) html += '<p class="text-xs text-zinc-500 italic mt-2">+' + (grouped.o.length - 5) + ' weitere Info-Meldungen</p>';
        html += '</div>';
      }
      act.innerHTML = html || '<p class="text-zinc-500 text-sm">' + t('noCritical') + '</p>';
    }
    ...
    if (data.html) {
      ...
      if (cwvPanel) {
        const metrics = [
          { label: 'LCP', val: cwv.lcp, unit: 'ms', good: 2500, warn: 4000, desc: 'Largest Contentful Paint' },
          { label: 'CLS', val: cwv.cls.toFixed(2), unit: '', good: 0.1, warn: 0.25, desc: 'Cumulative Layout Shift' },
          { label: 'FID', val: cwv.fid, unit: 'ms', good: 100, warn: 300, desc: 'First Input Delay' },
          { label: 'TTFB', val: cwv.ttfb, unit: 'ms', good: 600, warn: 1000, desc: 'Time to First Byte' },
        ];
        cwvPanel.innerHTML = metrics.map(m => {
          const isGood = m.val <= m.good;
          const isWarn = m.val <= m.warn;
          const color = isGood ? 'text-emerald-400' : isWarn ? 'text-amber-300' : 'text-red-400';
          const status = isGood ? '✓' : isWarn ? '⚠' : '✗';
          return '<div class="bg-void/40 rounded p-3"><div class="flex items-center justify-between mb-2"><p class="text-zinc-500 text-[10px] font-mono">' + m.label + '</p><span class="text-xs font-bold ' + color + '">' + status + '</span></div><p class="text-lg font-bold ' + color + '">' + m.val + m.unit + '</p><p class="text-[9px] text-zinc-600 mt-1">' + m.desc + '</p></div>';
        }).join('');
      }
      
      const a11yPanel = $('dashAccessibility');
      if (a11yPanel) {
        const a11yStatus = a11y >= 75 ? '✓ Excellent' : a11y >= 55 ? '⚠ Good' : '✗ Needs Work';
        const a11yColor = a11y >= 75 ? 'emerald' : a11y >= 55 ? 'amber' : 'red';
        const colorClass = a11y >= 75 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : a11y >= 55 ? 'bg-amber-500/10 border-amber-500/30 text-amber-300' : 'bg-red-500/10 border-red-500/30 text-red-400';
        a11yPanel.innerHTML = '<div class="inline-flex items-center justify-center w-28 h-28 rounded-full ' + colorClass + ' border-2 mb-4"><div class="text-center"><p class="text-3xl font-bold">' + a11y + '</p><p class="text-[10px] font-mono mt-1">WCAG 2.1</p></div></div><p class="text-sm font-semibold ' + colorClass.split(' ')[2] + ' mb-1">' + a11yStatus + '</p><p class="text-xs text-zinc-500 text-center">Barrierefreiheits-Score basierend auf DOM-Analyse</p>';
      }
      
      const resPanel = $('dashResources');
      if (resPanel) {
        const total = res.scripts + res.stylesheets + res.images + res.fonts;
        const items = [
          { name: 'Scripts', count: res.scripts, icon: '⚙️' },
          { name: LANG === 'en' ? 'Stylesheets' : 'Stylesheets', count: res.stylesheets, icon: '🎨' },
          { name: LANG === 'en' ? 'Images' : 'Bilder', count: res.images, icon: '🖼️' },
          { name: 'Fonts', count: res.fonts, icon: '🔤' },
        ];
        resPanel.innerHTML = items.map(item => '<div class="flex items-center justify-between py-2 border-b border-zinc-800/50 last:border-b-0"><div class="flex items-center gap-2"><span class="text-lg">' + item.icon + '</span><span class="text-sm text-zinc-400">' + item.name + '</span></div><span class="text-sm font-mono font-bold text-lime">' + item.count + '</span></div>').join('') + '<div class="flex items-center justify-between py-3 mt-3 pt-3 border-t-2 border-lime/30"><span class="text-sm font-semibold text-white">' + (LANG === 'en' ? 'Total Resources' : 'Gesamt-Ressourcen') + '</span><span class="text-lg font-mono font-bold text-lime">' + total + '</span></div>';
      }
    }
  ...
  async function loadDashHistory() {
    ...
    if (!ul) return;
    ul.innerHTML = '<li class="text-zinc-500 text-xs">Lade Audit-Verlauf...</li>';
    try {
      const { data, error } = await sb.from('audits').select('url, created_at, scores_json').order('created_at', { ascending: false }).limit(12);
      ul.innerHTML = '';
      if (error) {
        ul.innerHTML = '<li class="text-amber-400 text-xs">Verlauf nicht verfügbar</li>';
        return;
      }
      if (!data || data.length === 0) {
        ul.innerHTML = '<li class="text-zinc-500 text-xs italic">Noch keine Audits</li>';
        return;
      }
      (data || []).forEach((row) => {
        const sc = row.scores_json?.total ?? row.scores_json?.primary?.total ?? '—';
        const url = row.url || 'Unbekannt';
        const date = row.created_at ? new Date(row.created_at).toLocaleDateString(LANG === 'en' ? 'en-US' : 'de-DE') : '';
        const li = document.createElement('li');
        li.className = 'text-xs border-b border-zinc-800/50 pb-2 mb-2 last:border-b-0';
        li.innerHTML = `<div class="flex items-center justify-between mb-1"><span class="font-mono font-bold text-lime">${sc}</span><span class="text-zinc-500 text-[10px]">${date}</span></div><span class="text-zinc-400 truncate block">${escapeHtml(url)}</span>`;
        ul.appendChild(li);
      });
    } catch (e) {
      ul.innerHTML = '<li class="text-amber-400 text-xs">Fehler beim Laden</li>';
    }
  }
  ...
  function showPaywallModal(currentCount) {
    let modal = $('paywallModal');
    if (!modal) {
      const div = document.createElement('div');
      div.id = 'paywallModal';
      div.className = 'fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm hidden';
      div.innerHTML = '<div class="bg-zinccard border border-zinc-800 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl"><div class="flex items-center justify-center w-12 h-12 rounded-full bg-lime/10 mb-6 mx-auto"><i data-lucide="lock" class="w-6 h-6 text-lime"></i></div><h2 class="text-2xl font-bold text-white mb-2 text-center">Limit erreicht</h2><p class="text-sm text-zinc-400 mb-6 text-center">Du hast deine 5 kostenlosen Audits aufgebraucht. Upgrade auf Pro fuer unbegrenzte Audits und Premium-PDFs.</p><div class="bg-void/40 rounded-lg p-4 mb-6"><p class="text-xs text-zinc-500 uppercase tracking-wider mb-2">Dein Plan</p><div class="flex items-center justify-between"><span class="text-sm font-semibold text-white">Free</span><span class="text-xs font-mono text-amber-300">' + currentCount + '/5 Audits</span></div></div><button id="btnUpgradeStripe" class="w-full rounded-xl bg-lime text-void px-4 py-3 text-sm font-bold hover:brightness-95 transition mb-3 flex items-center justify-center gap-2"><i data-lucide="credit-card" class="w-4 h-4"></i> Jetzt upgraden</button><button id="btnClosePaywall" class="w-full rounded-xl border border-zinc-700 text-zinc-300 px-4 py-2 text-sm font-medium hover:bg-zinc-800 transition">Spaeter</button></div>';
      document.body.appendChild(div);
      icons();
      modal = $('paywallModal');
    }
    modal.classList.remove('hidden');
    $('btnUpgradeStripe')?.addEventListener('click', () => {
      const stripeCheckoutUrl = 'https://buy.stripe.com/YOUR_STRIPE_LINK_HERE';
  ...
  function updateAuditCounter(currentCount, plan) {
    const counter = $('auditCounter');
    if (counter) {
      const remaining = plan === 'free' ? Math.max(0, 5 - currentCount) : 999;
      const text = LANG === 'en' ? 'Audits: ' : 'Audits: ';
      const limit = plan === 'free' ? '5' : '∞';
      counter.textContent = text + currentCount + '/' + limit;
      const colorClass = plan === 'pro' ? 'text-lime' : remaining > 2 ? 'text-emerald-400' : remaining > 0 ? 'text-amber-300' : 'text-red-400';
      counter.className = 'text-xs font-mono ' + colorClass;
    }
  }
